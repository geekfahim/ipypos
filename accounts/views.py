...
def customerAj(request):
	...
	# 		address=address
	# 	)
	# data = {'id':obj.id,'name':obj.name, 'email':obj.email,'address':obj.address}	
	# return HttpResponse(data)	

    # request should be ajax and method should be POST.
    if request.is_ajax and request.method == "GET":
    	# customer = Customer.objects.filter(name)
    	
   		return JsonResponse({"Getting me..":customer},safe=False)
    ...
    if request.is_ajax and request.method == "POST":
        ...
        if form.is_valid():
            ...
            return JsonResponse("instance", status=200,safe=False)
        else:
